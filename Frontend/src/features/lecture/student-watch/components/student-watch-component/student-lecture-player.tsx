...
const StudentLecturePlayer = ({
  ...
  useEffect(() => {
    ...
    const reportPlaybackError = (message: string): void => {
    ...
    if (Hls.isSupported()) {
      const hls = new Hls({
        xhrSetup: xhr => {
          ...
          // HLS.js internally requests master file and segment files.
          xhr.open = function (
            method: string,
            url: string | URL,
            async?: boolean,
        ...
        fetchSetup: (context, initParams) => {
          // Some browsers/loaders use fetch instead of XHR for HLS requests.
          // We append signed query there too so CloudFront can allow segments.
          const signedUrl = appendSignedQuery({
            url: context.url,
            signedQuery: playback.signedQuery
          })
