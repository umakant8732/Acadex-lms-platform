...
export interface UploadButtonClickArgs {
  section: Section
  lesson: Lesson
}

export interface LectureCurriculumPageState {
  courseId: string | undefined
  curriculum: TeacherCourseCurriculum | null
  isLoading: boolean
  isError: boolean
  error: Error | null
  refetchCurriculum: () => void

  handleUploadButtonClick: (args: UploadButtonClickArgs) => void
...
export const useLectureCurriculumPage = (): LectureCurriculumPageState => {
  const { courseId } = useParams<{ courseId: string }>()
  const videoInputRef = useRef<HTMLInputElement | null>(null)
  const [uploadingLessonIds, setUploadingLessonIds] = useState<string[]>([])
  const [selectedUploadTarget, setSelectedUploadTarget] = useState<UploadButtonClickArgs | null>(null)
  const [uploadProgressByLessonId, setUploadProgressByLessonId] = useState<Record<string, number>>({})
  const [selectedPlayback, setSelectedPlayback] = useState<PlaybackAccessResult | null>(null)
  ...
  const handleUploadButtonClick = ({ section, lesson }: UploadButtonClickArgs) => {
  ...
  const handleWatchLecture = async (lesson: Lesson) => {
  ...
  return {
    ...
    error: curriculumQuery.error as Error | null,
    refetchCurriculum: () => { void curriculumQuery.refetch() },
