...
export interface AppendSignedQueryArgs {
  url: string
...
const appendSignedQuery = ({ url, signedQuery }: AppendSignedQueryArgs): string => {
  const alreadySigned =
    url.includes('Policy=') ||
...
export interface LecturePreviewPlayerProps {
  playbackAccess: PlaybackAccessResult | null
  onClose: () => void
  onPlaybackError?: (message: string) => void
}

const LecturePreviewPlayer: React.FC<LecturePreviewPlayerProps> = ({
  playbackAccess,
  onClose,
  onPlaybackError
}) => {
    ...
    if (Hls.isSupported()) {
      ...
      const hls = new Hls({
        ...
        fetchSetup: (context, initParams) => {
          ...
          const signedUrl = appendSignedQuery({
            url: typeof context === 'string' ? context : (context as any).url,
            signedQuery: playback.signedQuery
          })
