...
import type { StudentCurriculumSection } from '../../shared/types/student-course-types'
import type {
  StudentCourseContentStats,
  StudentCourseOverviewPageState
} from '../types/student-course-overview-types'
import { useGetStudentCourseCurriculum } from '../../../lecture/student-watch/queries/use-get-student-course-curriculum'
import { useGetStudentCourseOverview } from '../queries/use-get-student-course-overview'

const EMPTY_SECTIONS: StudentCurriculumSection[] = []

// Finds first ready preview lesson for non-purchased students.
const findFirstPreviewLesson = (sections: StudentCurriculumSection[]) => {
...
export const useStudentCourseOverviewPage = (): StudentCourseOverviewPageState => {
  const { courseId } = useParams<{ courseId: string }>()
  const normalizedCourseId = courseId ?? ''

  const studentCourseOverviewQuery = useGetStudentCourseOverview(normalizedCourseId)
  const studentCourseCurriculumQuery = useGetStudentCourseCurriculum(normalizedCourseId)
  ...
  const error = studentCourseOverviewQuery.error ?? studentCourseCurriculumQuery.error
  ...
  const primaryActionPath = isPurchased
    ? `/student/courses/${normalizedCourseId}/learn`
    : `/student/checkout/${normalizedCourseId}`
