...
import { useChangePublishStatus } from "../queries/use-change-publish-status";
import { useCreateCourseThumbnailPresignedUploadUrl } from "../queries/use-create-course-thumbnail-presigned-upload-url";
import { useCompleteCourseThumbnailUpload } from "../queries/use-complete-course-thumbnail-upload";
import { useDeleteCourse } from "../queries/use-delete-course";
import {
  allowedCourseThumbnailMimeTypes,
  maxCourseThumbnailUploadSize,
} from "../constants/course-thumbnail-upload";
import { uploadCourseThumbnailToS3Service } from "../services/service-upload-course-thumbnail-to-s3";
...
export const useManageCourseActions = () => {
  ...
  const handleThumbnailFileSelect = async (course, file) => {
    ...
    try {
      ...
      showError(getApiErrorMessage(error, "Unable to upload course thumbnail"));
    } finally {
